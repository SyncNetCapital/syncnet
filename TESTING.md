...
| Project Home · contracts (Foundry) | `tests/project-home/foundry.mjs` | `forge test` in `contracts/project-home-sink`: sink 60/40 forwarding to the converter; converter treasury-only execution, minOut, deadline, router revert, shallow pool, USDG failure, route change, re-entry, no admin/rescue/generic-swap surface; deploy gate. Needs `forge` (`npm i -g @foundry-rs/forge`); `SOLC_PATH` may point at a local solc 0.8.28. |
| Project Home · contract audit | `tests/project-home/sink-static-audit.mjs` | Source and ABIs of the sink and converter prove there is no owner, setter, rescue, sweep, withdrawal, delegatecall, upgrade, arbitrary call, recipient parameter, generic swap or payable surface. The sink has no DEX logic, and the canonical route is pinned in the deploy gate. |
| Project Home · pricing | `tests/project-home/pricing.test.mjs` | $39 USD → SYNC in BigInt fixed point (round up), rate/price versions, payment tag, fail-closed gate. |
