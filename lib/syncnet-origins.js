 ...
 *   PONS_V1  PonsLaunchFactory 0xA5aAb3F0c6EeadF30Ef1D3Eb997108E976351feB — detected positively, NOT supported
 *            (different economics and fee routing; see contractsV1/src/interfaces/ILaunchpad.sol).
 *
...
(function (root, factory) {
  ...
  /** Pons V1 factory record (positive identification only), or null. Throws when unreadable. */
  async function readPonsV1(rpc, token) {
    const t = lc(token);
    const hex = await call(rpc, PONS_V1_FACTORY, enc(SEL.getLaunchedToken, ['address'], [t]));
    if (!hex) return null;
    const r = Core.abiDecode([PONS_V1_RECORD], hex)[0];
    if (r[11] !== true || lc(r[0]) !== t) return null;
    return { token: lc(r[0]), deployer: lc(r[1]), pairedToken: lc(r[2]) };
  }
  ...
  async function resolveProject(rpc, token, opts) {
    ...
    if (v1) return { origin: 'PONS_V1', label: ORIGINS.PONS_V1.label, supported: false, token: t, factory: PONS_V1_FACTORY, deployer: v1.deployer };
    return null;
  }
  ...
  return Object.freeze({
    PONS_V2_FACTORY, PONS_V1_FACTORY, PAR_FACTORIES, FEE_FACTORIES, ORIGINS, PONS_PHASES, SEL, PENDING_GRACE_SECONDS,
