...
(function (root, factory) {
  ...
  const safeText = (v, max) => esc(Core.sanitizeForDisplay(String(v == null ? '' : v), { maxLength: max || 160 }));
  ...
  function link(l, enabled) {
    const host = esc(l.host) + (l.idn ? ' (internationalised domain, shown in ASCII form)' : '');
    if (!enabled) return `<span class="ext disabled" aria-disabled="true">${esc(l.label)}</span><span class="host">${host} · link disabled until the current Passport operator confirms this site</span>`;
    return `<a class="ext" href="${esc(l.href)}" rel="nofollow noopener noreferrer ugc" target="_blank" referrerpolicy="no-referrer">${esc(l.label)}</a><span class="host">${host}</span>`;
  }
  ...
  function render(opts) {
    ...
    if (config) {
      ...
      if (S.hero && (config.headline || cta || hero)) {
        ...
        if (config.headline) out.push(`<p class="headline">${esc(config.headline)}</p>`);
  ...
  return Object.freeze({
    ...
    canonicalJson, configHash, typedData, digest, checkUrl, normalize, validate, externalLinks, render, escapeHtml: esc,
  });
});
