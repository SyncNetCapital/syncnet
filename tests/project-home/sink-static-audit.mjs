...
check('deploy script requires an explicit treasury kind and checks its code before broadcasting', /vm\.envUint\("TREASURY_EXPECT_EOA"\)/.test(deploy) && deploy.indexOf('checkTreasuryCode') < deploy.indexOf('vm.startBroadcast()') && /if \(expectEoa == 1 && codeLength != 0\) revert TreasuryHasCode/.test(checks));
check('the approved treasury address is not hard-coded in contracts, scripts or application code', !/65fac39a7a672afebba404aecddb34a1eddc879b/i.test(deploy + checks + fs.readdirSync(path.join(DIR, '../..')).filter((f) => f.endsWith('.js') || f.endsWith('.html')).map((f) => fs.readFileSync(path.join(DIR, '../..', f), 'utf8')).join('')));
